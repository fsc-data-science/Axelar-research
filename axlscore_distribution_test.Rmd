...
# AxlScore Distributions

Before implementing a scoring methodology for Axlscore, this piece looks at historical 
...
Note: In reality different senders from different source chains can be the same person. For the formal Axlscore app the user will link cross-chain sender addresses to get credit for multiple sender addresses.
...
# NOT RUN 
# axlscore <- shroomDK::auto_paginate_query(axlscore_query, api_key = readLines("api_key.txt"))
...
axlscore <- read.csv("axlscore-selecttoken-sender-method-amounts.csv", row.names = NULL)
# fix avalanch -> avalanche
axlscore$SOURCE_CHAIN <- gsub(pattern = "$avalanch", 
                              replacement = "avalanche", 
                              x = axlscore$SOURCE_CHAIN)
axlscore$DESTINATION_CHAIN <- gsub(pattern = "$avalanch",
                                   replacement = "avalanche", 
                                   x = axlscore$DESTINATION_CHAIN)
...
sender_stats <- axlscore %>% group_by(SENDER) %>% 
  summarise(
...
When cross-chain identification in the final Axlscore product is available (i.e., 
